...
import { generateContentPack } from '@/lib/contentPack/generateContentPack';
...
function parsePhotos(value: unknown): PhotoAnalysis[] {
  ...
  return value.slice(0, 50).map((item) => {
    ...
    return {
      ...
      keywords,
      status: record.status === 'analyzed' ? 'analyzed' : 'failed',
    };
  });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (photos.length === 0) {
      return NextResponse.json({ error: '분석이 끝난 사진이 없어 콘텐츠를 만들 수 없습니다.' }, { status: 400 });
    }
    const pack = await generateContentPack({
      photos,
      galmaetgil: parseMatches(body.galmaetgil),
    });
    return NextResponse.json(pack);
  } catch (error) {
    const message = error instanceof Error ? error.message : '콘텐츠를 만들지 못했습니다.';
